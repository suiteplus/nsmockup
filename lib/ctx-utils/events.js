'use strict';
var ss = require('../db-utils/suite-script'),
...
exports.userEvent = (recType, step) => {
    let scripts = ss.find('user-event', {records: [recType]}) || [];
...
exports.executeUserEvent = (event, etype, records) => {
    ...
    if (script && script.functions && script.functions[step]) {
        let code = script.functions[step] + `("${etype}")`;
        if (!context) {
...
exports.existsTriggerUserEvent = (recType) => {
    let events = ss.find('user-event', {records: [recType]});
