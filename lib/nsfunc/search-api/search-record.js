...
var validateFields = require('../../search-utils/validate-fields'),
...
exports.nlapiSearchRecord = (type, id, filters, columns) => {
    ...
    function findMetadata(recType) {
        return metaCache[recType] || (metaCache[recType] = $metadata.find(recType));
    }
    function findFieldMetadata(recType, fieldCode) {
        let recordMeta = findMetadata(recType),
            fieldCacheKey = recType + '.' + fieldCode;
        let field = metaFieldCache[fieldCacheKey];
        if (!field) {
            let result = _.where(recordMeta.fields, {code: fieldCode});
            metaFieldCache[fieldCacheKey] = (field = result[0]);
        }
        if (!field) {
            return false;
        } else {
    ...
    function findJoinColumns(opts) {
        ...
        let field = findFieldMetadata(recType, join);
    ...
    function getFieldValue(opts) {
        let fieldMeta = findFieldMetadata(opts.type, opts.code);
        if (fieldMeta.recordType && fieldMeta.recordType.indexOf('customlist') === 0){
            let listItem = $db(fieldMeta.recordType).chain().where({internalid: opts.value}).value();
            return listItem && listItem.length ? listItem[0].name : opts.value;
        } else {
            return opts.value;
        }
    }
                ...
                for (let n = 0; n < names.length; n++) {
                    ...
                    if (name === '$val') {
                        if (names.length === 1) {
                            ...
                            rawValues[col] = getFieldValue(opts);
                        } else {
