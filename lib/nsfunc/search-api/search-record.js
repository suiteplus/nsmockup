    ...
    if (filters) {
        ...
        for (let f = 0; f < filters_.length; f++) {
            ...
            if (typeof filter_ === 'string') {
                ...
                if (filter_.length <= 3) {
                    filter_ = new nlobjSearchFilter(filter_[0], null, filter_[1], filter_[2]);
                } else {
        ...
        items = items.filter(item => {
            for (let i = 0; i < filters_.length; i++) {
                ...
                if (typeof filter_ === 'string') {
                    ...
                    if (!valActual) {
                        return false;
                    } else if (filter_.operator === 'contains' && minimatch(valActual, valExpect)) {
                        return true; // FIXME - validate all fields before return
                    } else if (valActual != valExpect){
                        return false;
                    }
                }
            }
