...
exports.nlapiScheduleScript = (id, deployment, parameters) => {
    if (!id) {
        throw nlapiCreateError('SSS_SCRIPT_ARG_REQD');
    } else if (!deployment) {
        throw nlapiCreateError('SSS_DEPLOY_ARG_REQD');
    }
    ...
    if (typeof id === 'number' || !isNaN(parseInt(id))) {
        ...
        script = scripts.chain().filter({code: id, type: 'schedule'}).value();
    }
    ...
    if (!script) {
        throw nlapiCreateError('SSS_INVALID_SCRIPT_ARG', `not found ScheduleScript with id or internal id: "${id}"`);
    } else {
