...
exports.nlapiLookupField = (type, id, fields, text) => {
    ...
    var res = collection.chain().filter({internalid: id}).value();
    if (!res || !res.length) {
        ...
        for (let i = 0; i < fields.length; i++) {
            ...
            if (joinRegExp.test(field)) {
                let ss = field.split('.'),
                    code = ss[0];
                if (!joins[code]) {
                    let metaField = _.filter(meta.fields, {code}),
                    ...
                    if (meta_ && meta_.code) {
                        let collection = $db(meta_.code),
                            ...
                            data_ = collection.chain().filter({internalid: (isNaN(dataAux) ? dataId : dataAux)}).value();
