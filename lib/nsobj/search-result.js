 ...
 * @constructor
 */
exports.nlobjSearchResult = function nlobjSearchResult(type, id, rawResults, rawColumns) {
    this.type = type;
    this.id = id;
    this.rawValues = rawResults.values;
    this.rawTexts = rawResults.texts;
    ...
    this.getValue = function (name, join, summary) {
        ...
        if (join_) {
            if (!this.rawValues[join_]) {
                return '';
            } else {
                return this.rawValues[join_][name_] || '';
            }
        } else {
            let value = this.rawValues[name_] || '';
    ...
    this.getText = function (name, join, summary) {
        let isCol = name instanceof nlobjSearchColumn,
            ...
            join_ = isCol ? name.join : join;
        if (join_) {
            if (!this.rawTexts[join_]) {
                return '';
            } else {
                return this.rawTexts[join_][name_] || '';
            }
        } else {
            let value = this.rawTexts[name_] || '';
            if (typeof value !== 'string' && value.id) {
                return '' + value.id;
            } else {
                return '' + value;
